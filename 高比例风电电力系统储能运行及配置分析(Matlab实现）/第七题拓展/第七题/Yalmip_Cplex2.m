...
for t = 1:T
    ...
    % 功率平衡
    St = [St,-0.5 <= P_DE1(t) + P_Wind(t) + P_Bat(t) - Load(t) <= 0.5];
end
% SOC计算
St = [St, 0.2*S_Bat <= S_Bat*Bat(5) - sum(Pdis(1,1:96)/Bat(4) + Bat(4)*Pcha(1,1:96)) <= 0.95*S_Bat];
% St = [St, -S_Bat*0.1 <= -sum(Pdis/Bat(4) + Bat(4)*Pcha) <= S_Bat*0.1];
%% 目标
Object = 0; % 总成本
F = 0;
Tan = 0;
