function [Result,Cost,PDE,PWind,Loss] = Yalmip_Cplex(Load,Wind,mpc,tan,wind,loadloss)
%% 初始定义
yalmip;
T = 96; %调度周期
% 决策变量
...
P_Wind = sdpvar(1,T,'full');  %实际风机出力
P_loss = sdpvar(1,T,'full');  %弃负荷功率
%% 约束
St = [];
for t = 1:T
...
for t = 1:T
    ...
    %St = [St,-0.1 <= P_DE1(t) + P_DE3(t) + P_Wind(t) - (Load(t) - P_loss(t)) <= 0.1];
end
%% 目标
Object = 0; % 总成本
F = 0;
...
% 弃负荷
for t = 1:T
    Loss_cost = Loss_cost +  P_loss(t)*loadloss*0.25*10^3;
end
