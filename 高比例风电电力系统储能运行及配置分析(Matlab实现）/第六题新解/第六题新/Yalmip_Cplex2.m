...
%% 约束
St = [];
St = [St, 0<=S_Bat(1,1)<=1000000, 0<=P_limit(1,1)<=10000]; % 暂时不设置上限
St = [St, -P_limit <= P_Bat<= P_limit, 0 <= Pdis(1:T) <= P_limit, -P_limit <= Pcha(1:T) <=0];
for t = 1:T
    % 其他机组约束
    ...
    % 功率平衡
    St = [St,-0.1 <= P_DE1(t) + P_DE3(t) + P_Wind(t) + P_Bat(t) - Load(t) <= 0.1];
end
%% 目标
Object = 0; % 总成本
F = 0; Tan = 0; % 煤耗及碳排放成本
...
Pdis = value(Pdis);
Pcha = value(Pcha);
Cost = [value((F/1000)*1.5*700),value(Tan),value(WIND_yunxing),value(WIND_qifeng),value((1000*Bat(1)/(10*365))*P_limit),value((1000*Bat(2)/(10*365))*S_Bat),value(Object)/(sum(Load)*0.25)];
end
