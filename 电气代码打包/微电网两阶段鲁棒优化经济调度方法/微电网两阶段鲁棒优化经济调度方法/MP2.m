...
%% 构建矩阵
x=[us,um]';%第一阶段变量
y=[p_g,p_ch,p_dis,PDR,PDR1,PDR2,p_buy,p_sell]';%第二阶段变量 这里考虑了最恶劣情况
u=[p_pv,PL]';%不确定量。这里为确定量为最恶劣场景（子问题的解）

Q01=[eye(24),zeros(24,24)];%us 其中eye（24）指返回24*24的单位矩阵
Q02=[zeros(24,24),eye(24)];%um

Q1=[eye(24),zeros(24,168)];%分布式电源约束的系数矩阵（24*192） 公式2
Q2=[zeros(1,24),eta.*ones(1,24),-1/eta.*ones(1,24),zeros(1,120)];%储能调度前后能量约束（1*192） 公式6

Q31=[zeros(24,24),eye(24),zeros(24,144)];%p_cha 储能充电功率约束（24*192）公式4 
Q32=[zeros(24,48),eye(24),zeros(24,120)];%p_dis 储能放电功率约束（24*192）公式5

Q4=[zeros(24,24),eta.*tril(ones(24,24),0),-1/eta.*tril(ones(24,24),0),zeros(24,120)];%储能各时段剩余容量约束（24*192）公式7
Q51=[zeros(24,144),eye(24),zeros(24,24)];%p_buy 配电网购电约束（24*192）公式15
Q52=[zeros(24,168),eye(24)];%p_sell 配电网售电约束（24*192） 公式16

Q6=[eye(24),-eye(24),eye(24),-eye(24),zeros(24,24),zeros(24,24),eye(24),-eye(24)];%一个约束 公式25倒数第二行 Iu

%Q7=[zeros(24,72),eye(24),-eye(24)];

Q8=[zeros(1,72),ones(1,24),zeros(1,96)];%可转移负荷守恒约束（1，192） 公式8
Q9=[zeros(24,72),eye(24),zeros(24,96)];%可转移负荷上下限约束（24，192） 公式9

Q101=[zeros(24,96),eye(24),zeros(24,72)];%Pdr1约束（24，192） 公式13
Q102=[zeros(24,120),eye(24),zeros(24,48)];%Pdr2约束（24，192） 公式13
Q103=[zeros(24,72),eye(24),eye(24),-eye(24),zeros(24,48)];%期望功率守恒约束（24，192） 公式12

QCS=[zeros(24,24),KS*eta.*eye(24),KS*1/eta.*eye(24),zeros(24,120)];%平均充放电成本 公式3
QCDR=[zeros(24,96),KDR*eye(24),KDR*eye(24),zeros(24,48)];%需求响应调度成本 公式11
QCM=[zeros(24,144),eye(24),-eye(24)];%微电网配电网交易成本 公式17  
QC=[a*ones(1,24),KS*eta.*ones(1,24),KS*1/eta.*ones(1,24),zeros(1,24),KDR*ones(1,24),KDR*ones(1,24),price'.*ones(1,24),-price'.*ones(1,24)];%紧凑后的目标函数
